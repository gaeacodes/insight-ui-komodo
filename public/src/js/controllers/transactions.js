          ...
          function (e) {
            if (e.status === 400) {
              $rootScope.flashMessage =
                "Invalid Transaction ID: " + $routeParams.txId;
...
angular
  .module("insight.transactions")
  .controller("SendRawTransactionController", function ($scope, $http) {
    $scope.transaction = "";
    ...
    $scope.send = function () {
      ...
      $http
        .post(window.apiPrefix + "/tx/send", postData)
        .success(function (data, status, headers, config) {
          if (typeof data.txid != "string") {
            // API returned 200 but the format is not known
            $scope.status = "error";
            $scope.error =
              "The transaction was sent but no transaction id was got back";
            return;
          }
